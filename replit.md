...
- Level 5 "The Longest Afternoon" runs a heat-wave rhythm (`updateHeat()`, gated on `level.id === 5`): a calm→warn→flare phase machine whose `heatPulse` (eased 0..1) drives `heatDrainMult` (open-sun drain up to ~1.9x) and the `drawHeatWave` shimmer/flare-tint/telegraph-banner visuals. Shade and bat shields stay fully protective (mult only applies in the open-sun drain branch). `heatPhase`/`heatT`/`heatPulse`/`heatDrainMult` are part of the loadLevel reset contract above — `loadLevel()` re-arms them (`heatT` seeds a calm beat for L5, 0 elsewhere).
